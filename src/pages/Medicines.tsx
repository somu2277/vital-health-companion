    ...
    <div className="space-y-6">
      ...
      <div className="border border-border rounded-xl p-6 bg-card">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="font-semibold">Your Medicines ({medicines?.length || 0})</h2>
        </div>
        {isLoading ? (
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {medicines.map(med => (
              <div key={med.id} className="border border-border rounded-xl p-4 bg-background">
                ...
                {med.source_report && <p className="text-xs text-muted-foreground mt-2">From: {med.source_report}</p>}
              </div>
            ))}
          </div>
        ) : (
          <p className="text-center text-muted-foreground py-8">No medicines found. Upload a prescription to get started.</p>
        )}
      </div>
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground p-4 rounded-lg bg-warning/10">
        ...
        Always verify medicine availability and dosage with your pharmacist.
      </div>
    </div>
  );
}
