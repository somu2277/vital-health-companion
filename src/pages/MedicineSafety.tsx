        ...
        <div className="flex gap-3">
          <div className="relative flex-1">
            ...
            <input value={medicine} onChange={e => setMedicine(e.target.value)}
              ...
              placeholder={t("medicineSafety.enterMedicine")} className="w-full h-10 pl-10 pr-4 rounded-lg border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
          </div>
          <Button className="gap-2" onClick={checkSafety} disabled={loading || !medicine.trim()}>
            {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Shield className="h-4 w-4" />} {t("medicineSafety.checkSafety")}
          </Button>
        </div>
        ...
        {result ? (
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              ...
              <p className="text-sm text-muted-foreground">{t("medicineSafety.safetyScore")}</p>
            </div>
            {result.warnings.length > 0 && (
              <div>
                <h3 className="font-semibold mb-2 text-destructive">⚠️ {t("medicineSafety.warnings")}</h3>
                <ul className="space-y-1">{result.warnings.map((w, i) => <li key={i} className="text-sm text-muted-foreground">• {w}</li>)}</ul>
              </div>
            )}
            {result.interactions.length > 0 && (
              <div>
                <h3 className="font-semibold mb-2">{t("medicineSafety.interactions")}</h3>
            ...
            {result.side_effects.length > 0 && (
              <div>
                <h3 className="font-semibold mb-2">{t("medicineSafety.sideEffects")}</h3>
          ...
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p className="font-medium">{t("medicineSafety.enterToCheck")}</p>
          </div>
        )}
      </div>
